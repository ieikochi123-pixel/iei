...
function PersonCard({ name, role, detail, location }) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-md p-4">
      ...
      {location && (
        <p className="text-[var(--color-cyan)] text-xs mt-1">
          {location}
        </p>
      )}
    </div>
    ...
    <div className="bg-[var(--color-navy)]">
      ...
      <section className="relative py-28 px-5 md:px-8 text-center overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto">
          <img
            src="/logo.png"
            alt="IE(I) Kochi Local Centre Logo"
            className="mx-auto h-20 md:h-28 w-auto mb-6"
          />

          <span className="titleblock">
            39th National Convention &middot; IEI
          </span>
          ...
          <p className="text-white/60 mt-4">
            Theme: AI and IoT for an Inclusive and Empowered Society
          </p>
        </div>
      </section>
      ...
      <section id="about" className="py-16 px-5 md:px-8">
        <div className="max-w-6xl mx-auto">
          <SectionHeading
            eyebrow="Overview"
            title="About the 39th National Convention"
          />
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {aboutCards.map((card) => (
              <div
                key={card.title}
                className="border-l-4 border-[var(--color-brass)] bg-white/5 rounded-r-md p-6"
              >
                <h3 className="font-[var(--font-display)] text-lg text-[var(--color-brass-light)]">
                  {card.title}
                </h3>
      ...
      <section
        id="submit"
        className="py-16 px-5 md:px-8 bg-[var(--color-navy-deep)]/40"
      >
        <div className="max-w-3xl mx-auto text-center">
          <SectionHeading
            eyebrow="Call for Papers"
            title="Share Your Research - Note to Authors"
          />

          <p className="text-white/70 leading-relaxed">
            Authors are invited to submit original and unpublished research
            papers aligned with the convention&apos;s thematic areas, through
            the Microsoft CMT3 platform. Submissions undergo rigorous peer
            review; selected papers may be presented at the National Convention
            and considered for a SCOPUS-indexed Springer journal, subject to
            approval. Accepted authors remit Rs. 5000 after selection.
          </p>
          ...
          <table className="w-full text-sm border border-white/10 rounded-md overflow-hidden">
            <thead>
              <tr className="bg-[var(--color-navy-soft)] text-white">
                <th className="text-left py-3 px-4 font-mono text-xs tracking-widest uppercase">
                  Event
                </th>
              ...
              {importantDates.map((date) => (
                ...
                >
                  <td
                    className={`py-3 px-4 ${
                      date.highlight
                        ? 'text-[var(--color-brass-light)] font-semibold'
                        : 'text-white/80'
                    }`}
                  >
                    {date.event}
      ...
      <section className="py-16 px-5 md:px-8 bg-[var(--color-navy-deep)]/40">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {nationalAdvisoryCommittee.map((person) => (
              <PersonCard
                key={person.name}
                name={person.name}
                role={person.role}
      ...
      <section id="committee" className="py-16 px-5 md:px-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {organisingCommittee.map((person, index) => (
              <PersonCard
      ...
      <section className="py-16 px-5 md:px-8 bg-[var(--color-navy-deep)]/40">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {internationalTechnicalCommittee.map((person, index) => (
              <PersonCard
                key={`${person.name}-${index}`}
                name={person.name}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {nationalTechnicalCommittee.map((person, index) => (
              <PersonCard
                key={`${person.name}-${index}`}
                name={person.name}
                detail={person.detail}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            {tracks.map((track) => (
              <div
                key={track.title}
                className="bg-white/5 border border-white/10 rounded-md p-6 text-center"
              >
                <div className="text-4xl mb-3">
                  {track.icon}
                </div>

                <h3 className="font-[var(--font-display)] text-white">
                  {track.title}
                </h3>
